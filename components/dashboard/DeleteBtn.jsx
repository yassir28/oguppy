...
export default function DeleteBtn({id, endpoint}) {
    ...
    async function handleDelete () {
        ...
        if (result.isConfirmed) {
            const res = await fetch(`${baseUrl}/api/${endpoint}?id=${id}`, {method:"DELETE"})
            if (res.ok){
    ...
    return (
        ...
        {loading ? (
            <button
                ...
                font-medium rounded-lg text-[0.5rem] px-5 py-2.5 text-center 
                mr-2 dark:bg-red-600 dark:hover:bg-red-700 
